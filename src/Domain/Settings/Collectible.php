    ...
    private function findCssVariable(string $slug_or_default): string
    {
        ...
        if (false !== \strpos($slugs_or_default[0], '.')) {
            ...
            foreach ($this->collection_of_collections as $collection) {
                /**
                 * @TODO If category is not found, then add some error message to console
                 *       This happens when the category is not found in the collection
                 *       Example: "color.primary" is not found in "palette" collection
                 */
